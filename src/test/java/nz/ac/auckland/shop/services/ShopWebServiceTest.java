...
//		assertEquals(newPurchase, purchasesFromService.get(0));
	...
	public void addCustomerCreditCard() {
		...
		Response response = _client.target(WEB_SERVICE_URI + "/3/creditCards")
				.request().post(Entity.xml(creditCard));
		if (response.getStatus() != 204) {
			fail("Failed to create new Credit Card");
		}
		response.close();

		Set<CreditCard> ccFromService = _client.target(WEB_SERVICE_URI + "/3/creditCards")
				.request().accept("application/xml").get(new GenericType<Set<CreditCard>>() {});
